	...
	*/
	class Nav extends Common{
		...
		public function deleteMenuDatas(){
			...
			$id = $data['id'];
			$result =  Db::Connect('db_config1')->name('nodes')->where('id',$id)->delete();
			return $result;
		}
		...
		public function addMenuDatas(){
			$data = $this->request->post();
			 Db::Connect('db_config1')->name('nodes')->insert($data);
		}
		...
		public function editMenuDatas(){			
			...
			$result =  Db::Connect('db_config1')->name('nodes')->where('id',$treeNodeId)->data(['name'=>$newName])->update();
		...
		public function findhis(){
			...
			//根据序列号查找到表明
			$result = Db::Connect('db_config1')->query("select name from nodes where Xlh = {$xlh}");
